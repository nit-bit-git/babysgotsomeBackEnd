...
const registerUser = asyncHandler( async (req,res)=> {
   ...
   if (req.files && Array.isArray(req.files.coverImage) && req.files.coverImage.length >0) 
    { coverImageLocalPath = req.files.coverImage[0].path }// [0] gives 1st property which can give us the path //  if file not retrieved check multer middleware for file naming issues
                                                                 // validate correct email format as well 
    if (!avatarLocalPath){
        throw new ApiError(400, "Avatar file is required")
    }
    // console.log("val: ", avatarLocalPath)
    ...
    // if (!coverImage){
    //     throw new ApiError(400, "cover file upload failed")
    // }
